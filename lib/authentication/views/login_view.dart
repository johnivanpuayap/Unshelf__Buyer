  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (!userDoc.exists) {
        ...
        _showSnack('User not found in database.');
        return;
      }

      final bool banned = (userDoc['isBanned'] as bool?) ?? false;
      if (banned) {
        await FirebaseAuth.instance.signOut();
        if (!mounted) return;
        _showSnack('Your account is banned. Please contact support.');
        return;
      }
      ...
      if (role != 'buyer') {
        await FirebaseAuth.instance.signOut();
        if (!mounted) return;
        _showSnack('User has a different role');
        return;
      }
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        TextButton(
                          ...
                          child: Text('Create an account', style: tt.labelLarge?.copyWith(color: cs.primary)),
                        ),
                      ],
                    ),
                  ],
